...
const Products: React.FC = () => {
  ...
  const tabs = ['All', 'Wigs', 'Braids', 'Extensions', 'Care'];

  React.useEffect(() => {
    storeApi.getProducts('all').then((result) => setRows(result));
  }, []);
                ...
                <tr key={product.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                         <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${
                             product.status === 'In Stock' ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'
                         }`}>
                             <span className={`size-1.5 rounded-full ${product.status === 'In Stock' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                             {product.status}
                         </span>
                    </td>
