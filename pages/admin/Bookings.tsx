...
const Bookings: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = React.useState<'all' | Booking['status']>('all');
  ...
  const currentDate = new Date(2026, 3 + monthOffset, 1);
    ...
    <div className="p-6 lg:p-10 space-y-6 max-w-[1600px] mx-auto h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)} className="rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-4 py-2.5 text-sm">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-2xl p-6 shadow-sm flex flex-col">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <p className="text-sm text-slate-500">No appointments match the current filter.</p>
            ) : (
                ...
                <div key={booking.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="min-w-[120px]">
                      ...
                      <div className={`text-xs font-bold px-2 py-0.5 rounded-full w-fit ${booking.status === 'Confirmed' ? 'bg-emerald-100 text-emerald-700' : booking.status === 'Completed' ? 'bg-slate-200 text-slate-700' : 'bg-amber-100 text-amber-700'}`}>
                        {booking.status}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex gap-4 items-center">
                    {booking.avatar ? (
                      ...
                      <div className="size-10 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center" aria-hidden>
      ...
      >
        {editingBooking && (
          <div className="space-y-3">
            ...
            <select value={editingBooking.status} onChange={(e) => setEditingBooking((prev) => (prev ? { ...prev, status: e.target.value as Booking['status'] } : prev))} className="w-full rounded-lg border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-2.5">
