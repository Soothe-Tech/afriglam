          ...
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-white/5 p-6 lg:p-8 shadow-sm">
                  ...
                  <div className="space-y-6">
                      <div className="flex items-center gap-6">
                          <div className="relative">
                              <img src={profileImageUrl || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(profile.firstName + ' ' + profile.lastName || 'Admin')} className="size-24 rounded-full object-cover border-4 border-slate-50 dark:border-slate-800" alt="Profile" />
              ...
              {activeTab === 'notifications' && (
                  <div className="space-y-6">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 text-sm">Email and in-app notification preferences. Configure which updates you want to receive (order alerts, low stock, new bookings).</p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs">Notification settings will be available in a future update.</p>
                  </div>
              )}
              ...
              {activeTab === 'store' && (
                  <div className="space-y-6">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 text-sm">Store name, currency defaults, and regional options. These affect how your store appears to customers.</p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs">Store-level settings will be available in a future update.</p>
                  </div>
              )}
              ...
              {activeTab === 'security' && (
                  <div className="space-y-6">
                      <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg flex gap-3">
                          ...
                          <button onClick={() => { setTwoFactorEnabled((prev) => !prev); setAlertState({ isVisible: true, type: 'info', message: !twoFactorEnabled ? '2FA will be enabled after save.' : '2FA will be disabled after save.' }); }} className="ml-auto text-xs font-bold bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-100 px-3 py-1 rounded hover:bg-amber-200 dark:hover:bg-amber-500/30 transition-colors">{twoFactorEnabled ? 'Disable' : 'Enable'}</button>
                      </div>
              ...
              <div className="pt-8 mt-8 border-t border-slate-100 dark:border-white/5 flex justify-end">
                  <button disabled={saving}
                    ...
                    className="px-6 py-3 bg-admin-primary hover:bg-emerald-600 disabled:opacity-60 text-white font-bold rounded-lg shadow-lg shadow-admin-primary/20 transition-all active:scale-95"
                  >
