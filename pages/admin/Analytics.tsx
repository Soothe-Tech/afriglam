    ...
    <div className="p-6 lg:p-10 space-y-8 max-w-[1600px] mx-auto animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
            ...
            <button 
                ...
                className="flex items-center gap-2 bg-admin-primary hover:bg-emerald-600 text-white px-4 py-2.5 rounded-lg transition-colors shadow-lg shadow-admin-primary/20"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {[
             ...
             { title: 'Conversion Rate', value: '3.2%', change: '-0.4%', isPositive: false },
             { title: 'Avg. Order Value', value: '380 PLN', change: '+5%', isPositive: true },
         ].map((stat, idx) => (
              ...
              <div className="h-64 flex items-end justify-between gap-2">
                  {[35, 45, 30, 60, 75, 50, 65, 80, 55, 70, 90, 60].map((h, i) => (
                      <div key={i} className="w-full bg-slate-100 dark:bg-white/5 rounded-t-sm relative group">
                          <div 
                            style={{ height: `${h}%` }} 
                            className="absolute bottom-0 w-full bg-admin-primary opacity-80 group-hover:opacity-100 transition-all rounded-t-sm"
                          ></div>
                      </div>
                  ))}
              </div>
      ...
      <Modal 
        ...
        actions={
            <>
                ...
                <button disabled={isExporting} onClick={downloadExport} className="px-4 py-2 rounded-lg bg-admin-primary hover:bg-emerald-600 text-white transition-colors text-sm font-bold disabled:opacity-60">Download CSV</button>
            </>
        }
      >
