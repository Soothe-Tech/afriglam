...
const ProductDetail: React.FC = () => {
  ...
  React.useEffect(() => {
    if (!id) return;
    setError('');
    storeApi.getProductById(id).then((result) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 xl:gap-20">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6 h-fit sticky top-24">
          <div className="flex-1 aspect-[3/4] md:aspect-auto md:h-[700px] bg-neutral-100 dark:bg-white/5 rounded-2xl overflow-hidden relative group">
            ...
            <div className="absolute top-4 left-4">
               <span className="bg-white/90 dark:bg-black/60 backdrop-blur text-primary dark:text-white text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider border border-white/20">Best Seller</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-8">
          <div className="space-y-4 border-b border-gray-100 dark:border-white/10 pb-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="font-mono text-2xl text-primary dark:text-admin-primary font-bold tracking-tight">
                ₦{product.price_ngn.toLocaleString()} <span className="text-gray-400 font-normal mx-2 text-xl">/</span> {product.price_pln} PLN
              </div>
          ...
          <p className="text-gray-600 dark:text-gray-300 text-base leading-relaxed">
            Elevate your protective style game with our premium fibers designed for a natural, lightweight look. Soft texture, tangle-free, and easy to install for the perfect finish.
          </p>
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <div className="flex gap-3 flex-wrap">
                {['#0a0a0a', '#3e2723', '#560015', '#b8860b'].map((hex) => (
                  <button
                    key={hex}
                    type="button"
                    onClick={() => setSelectedColor(hex)}
                    className={`group relative size-12 rounded-full ring-2 ring-offset-2 dark:ring-offset-background-dark transition-all focus:ring-primary ${selectedColor === hex ? 'ring-primary' : 'ring-transparent hover:ring-gray-300'}`}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <div className="flex items-center justify-between px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-full w-full sm:w-32 bg-white dark:bg-white/5">
                <button onClick={() => setQuantity((prev) => Math.max(1, prev - 1))} className="text-gray-400 hover:text-primary transition-colors" aria-label="Decrease quantity"><span className="material-symbols-outlined !text-xl">remove</span></button>
                <span className="font-mono text-lg font-bold">{quantity}</span>
                <button onClick={() => setQuantity((prev) => prev + 1)} className="text-gray-400 hover:text-primary transition-colors" aria-label="Increase quantity"><span className="material-symbols-outlined !text-xl">add</span></button>
              </div>
              <button onClick={handleAddToCart} className="flex-1 bg-primary hover:bg-primary-hover text-white rounded-full py-4 px-8 font-bold tracking-wide shadow-xl shadow-primary/20 hover:shadow-primary/40 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 group">
